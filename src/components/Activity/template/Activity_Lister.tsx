...
import getActivityLister from "../../../helper/activity/getActivitiesLister";
import getActivityListerByStatus from "../../../helper/activity/getActivitiesListerByStatus";

import ActivityCardList from "../organisms/ActivityCardList";
import TagListItem from "../../Home/organisms/TagList";

export default function ActivityScreen_Lister({ navigation }: any) {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchData = async () => {
        try {
          const activityData = await getActivityLister(user.id);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (selectedTag === "All") {
          activityData = await getActivityLister(user.id);
        } else {
          const activityData = await getActivityListerByStatus(
            user.id,
            selectedTag
          );
          activityData && setActivityList(activityData);
